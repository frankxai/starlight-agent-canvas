...
`pnpm mcp:codex:smoke` proves the Codex installer path without mutating your real Codex config: it writes a temporary `config.toml`, runs `doctor --config` against that file, and removes the temp files.
