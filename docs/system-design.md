...
Portable JSON import validates the same canvas record schema used by exports. The web app previews the selected file before mutation, including counts, node kinds, sample nodes, and same-id conflict status. Imports preserve the incoming id when it is new to the local home; when that id already exists, the store creates a non-destructive copy with a fresh canvas id and updated timestamps.
...
`note`, `source_url`, `source_pdf`, `source_youtube`, `source_video`, `source_image`, `prompt`, `mcp_tool`, `agent_run`, `output`
...
The MCP server exposes safe local tools only. It can list/get/create/import canvases, add/update/ingest positioned nodes, ingest text/URL/YouTube/video/image/PDF sources, connect nodes, run actions, search node/artifact/chunk evidence, and export. It never posts, pays, scrapes social platforms, or deletes data.

## Network Boundary

The Next.js API is localhost-only unless `AGENT_CANVAS_ALLOW_REMOTE=1` is set. URL ingestion rejects localhost/private networks and unsupported schemes, applies timeout and byte limits, and uses Firecrawl only when explicitly requested. PDF ingestion validates type and size before parse. Image ingestion accepts PNG, JPEG, WebP, GIF, and AVIF under a bounded size limit, stores local uploads as data URLs in the user's private canvas home, and stores remote image URLs as references without fetching arbitrary binary content. YouTube ingestion is transcript-first: manual transcript, best-effort public captions, then metadata/reference fallback. Generic video ingestion is reference-first: Loom, Vimeo, Wistia, TikTok, Drive, Dropbox, and direct video links become `source_video` nodes with `video` artifacts, optional manual transcript/notes, chunks, and provenance, without downloading or rehosting media.
