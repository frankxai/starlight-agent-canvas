...
- Right rail: selected-context tray, handoff readiness lane, source-grounded ask box, action drawer, editable selected node inspector with context receipt and selected-source commands immediately after the action drawer, setup/MCP status, local search, run log.
