...
type IngestSourceResult = Awaited<ReturnType<FileCanvasStore['ingestSource']>>;
...
export function createToolHandlers(store = new FileCanvasStore()) {
  ...
  async function withSingleSourceTrace(
    canvasId: string,
    result: IngestSourceResult,
    input: { sourceLabel: string; origin: string; detectedKind: string; inputSummary?: string },
  ): Promise<IngestSourceResult & { trace: unknown; sourceReadiness: unknown }> {
    const { trace, sourceReadiness } = createIntakeTraceForNodes({
      ...
      inputSummary: input.inputSummary ?? result.node.title,
      inputChars: result.artifact.body.length,
      detectedKinds: [input.detectedKind],
      urls: result.artifact.source ? [result.artifact.source] : [],
    });
    const canvas = await store.appendIntakeTrace(canvasId, trace);
    return { ...result, canvas, trace, sourceReadiness };
  }
  ...
  return {
    ...
    async ingest_text_source(args: {
      ...
      const traced = await withSingleSourceTrace(canvasId, result, {
        sourceLabel: 'MCP text source',
        origin: 'mcp_ingest_text_source',
        detectedKind: 'text',
        inputSummary: filename ?? result.node.title,
      });
      return ok(`Ingested text source ${traced.node.title} (${traced.node.id})`, traced);
    },
    ...
    async ingest_url(args: { canvasId: string; url: string; useFirecrawl?: boolean; title?: string; position?: { x: number; y: number } }): Promise<ToolResult> {
      ...
      const traced = await withSingleSourceTrace(args.canvasId, result, {
        sourceLabel: 'MCP URL source',
        origin: 'mcp_ingest_url',
        detectedKind: 'url',
        inputSummary: args.url,
      });
      return ok(`Ingested URL source ${traced.node.title} (${traced.node.id})`, traced);
    },
    ...
    async ingest_youtube(args: { canvasId: string; url: string; manualTranscript?: string; title?: string; position?: { x: number; y: number } }): Promise<ToolResult> {
      ...
      const traced = await withSingleSourceTrace(args.canvasId, result, {
        sourceLabel: 'MCP YouTube source',
        origin: 'mcp_ingest_youtube',
        detectedKind: 'youtube',
        inputSummary: args.url,
      });
      return ok(`Ingested video source ${traced.node.title} (${traced.node.id})`, traced);
    },
    ...
    async ingest_video(args: { canvasId: string; url: string; manualTranscript?: string; title?: string; position?: { x: number; y: number } }): Promise<ToolResult> {
      ...
      const traced = await withSingleSourceTrace(args.canvasId, result, {
        sourceLabel: 'MCP video source',
        origin: 'mcp_ingest_video',
        detectedKind: 'video',
        inputSummary: args.url,
      });
      return ok(`Ingested video reference ${traced.node.title} (${traced.node.id})`, traced);
    },
    ...
    async ingest_image(args: {
      ...
      if (args.url) {
        ...
        const traced = await withSingleSourceTrace(args.canvasId, result, {
          sourceLabel: 'MCP image source',
          origin: 'mcp_ingest_image',
          detectedKind: 'image',
          inputSummary: args.url,
        });
        return ok(`Ingested image reference ${traced.node.title} (${traced.node.id})`, traced);
      }
      ...
      const traced = await withSingleSourceTrace(args.canvasId, result, {
        sourceLabel: 'MCP image upload',
        origin: 'mcp_ingest_image_upload',
        detectedKind: 'image',
        inputSummary: args.filename,
      });
      return ok(`Ingested image source ${traced.node.title} (${traced.node.id})`, traced);
    },
    ...
    async ingest_pdf(args: { canvasId: string; filename: string; dataBase64: string; position?: { x: number; y: number } }): Promise<ToolResult> {
      ...
      const traced = await withSingleSourceTrace(args.canvasId, result, {
        sourceLabel: 'MCP PDF source',
        origin: 'mcp_ingest_pdf',
        detectedKind: 'pdf',
        inputSummary: args.filename,
      });
      return ok(`Ingested PDF source ${traced.node.title} (${traced.node.id})`, traced);
    },
    ...
    async ingest_anything(args: {
      ...
      const prompt = args.prompt
        ?? (action === 'answer_question'
          ...
          : '');
      const result = await mapSourceIntakeToCanvas(store, canvasId, args.content, {
        origin: 'mcp_ingest_anything',
        sourceLabel: 'MCP intake',
        title: args.title,
        position: args.position,
        action,
        prompt,
      ...
      const results = result.nodes.map((node, index) => ({
        kind: result.plan.items[index]?.kind ?? node.kind,
        node,
        artifact: result.artifacts[index],
      }));

      return ok(`Mapped ${results.length} item(s) into ${canvasId}: ${results.map((item) => item.kind).join(', ')}.`, {
        canvasId,
        canvas: result.canvas,
        detected: result.plan,
        detectedSummary: result.detected,
        results,
        nodeIds,
        sourceReadiness: result.sourceReadiness,
        trace: result.trace,
        run: result.run ? { canvas: result.canvas, run: result.run, outputNode: result.outputNode } : undefined,
      });
    },
